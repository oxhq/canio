...
To validate against the published Packagist package and the published `v1.0.6` GitHub release assets instead of local sources:

```bash
CANIO_PACKAGE_SOURCE_MODE=packagist \
CANIO_PACKAGE_CONSTRAINT=v1.0.6 \
CANIO_RUNTIME_RELEASE_SOURCE=github \
CANIO_RUNTIME_RELEASE_VERSION=v1.0.6 \
./scripts/smoke-launch.sh
```
